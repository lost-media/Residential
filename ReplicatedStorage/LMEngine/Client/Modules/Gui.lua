...
---@type Maid
local Maid = require(LMEngine.GetShared("Maid"))
...
export type Screen = {
	...
	ActiveScreen: Screen?,
	ActivePrompt: Screen?,
	OnDisable: LMEngine.Signal,
	OnActivate: LMEngine.Signal,
	ActiveScreenChanged: LMEngine.Signal,
	ActivePromptChanged: LMEngine.Signal,
	OnParented: LMEngine.Signal,
	_active_lock: number,
	_default_props: { [string]: any },
	_active_maid: Maid.Maid,
}
...
function Screen:Activate(props)
	...
	if parent ~= nil and parent._active_lock ~= 0 then
		warn(
			'[Gui]: Tried to enable ancestor/sibling screen "'
				.. self.Name
				.. '" inside OnActivate/OnDisable listener; Traceback:\n'
				.. debug.traceback()
		)
		return
	end
	...
	while ancestor ~= nil do
		...
		if ancestor.IsActive == false then
			warn(
				'[Gui]: Tried to activate screen ("'
					.. trace
					.. '") with an inactive ancestor; Traceback:\n'
					.. debug.traceback()
			)
			return
		end
...
function Screen:Disable(_is_changing_screens)
	...
	if parent ~= nil and parent._active_lock ~= 0 then
		warn(
			'[Gui]: Tried to disable ancestor/sibling screen "'
				.. self.Name
				.. '" inside OnActivate/OnDisable listener; Traceback:\n'
				.. debug.traceback()
		)
	...
	self.IsActive = false
	self.OnDisable:Fire()
	self._active_maid:DoCleaning()
	self._active_maid = Maid.new()
...
function Gui.Get(instance, name_array) --> [Instance] -- Throws an error if not found
	...
	if pointer ~= nil then
		...
		if instance == nil then
			...
			error(
				'[Gui]: Could not find "'
					.. tostring(name_array[index])
					.. '" inside of "'
					.. pointer_debug:GetFullName()
					.. '"'
			)
		end
	end
		...
		if target:IsA("GuiBase2d") == true and align ~= nil then
			...
			local function scale_update()
				...
				if target_size_udim2.X.Scale ~= 0 or target_size_udim2.Y.Scale ~= 0 then
					warn(
						"[Gui]: Expecting target to not use scale ("
							.. target:GetFullName()
							.. "); Traceback:\n"
							.. debug.traceback()
					)
...
function Gui.NewScreen(screen_params) --> [Screen]
	...
	local screen = {
		...
		OnDisable = LMEngine.NewScriptSignal(),
		OnActivate = LMEngine.NewScriptSignal(),
		ActiveScreenChanged = LMEngine.NewScriptSignal(),
		ActivePromptChanged = LMEngine.NewScriptSignal(),

		OnParented = LMEngine.NewScriptSignal(),
