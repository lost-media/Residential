...
local Signal = require(dir_Shared.Signal)
...
function KnitClient.CreateController(controllerDef: ControllerDef): Controller
	...
	assert(
		type(controllerDef.Name) == "string",
		`Controller.Name must be a string; got {type(controllerDef.Name)}`
	)
...
function KnitClient.GetController(controllerName: string): Controller
	...
	assert(
		...
		`ControllerName must be a string; got {type(controllerName)}`
	)
	error(
...
function KnitClient.Start(options: KnitOptions?)
	...
	KnitClient.AddControllers(dir_Controllers)
...
function KnitClient.GetShared(module_name: string)
	return SharedLoader:GetModule(module_name)
end

function KnitClient.GetModule(module_name: string)
