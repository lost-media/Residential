...
function Plot:PlaceStructure(structure: Model, state: PlacementType.ServerState)
	...
	if state._is_stacked == false then
		...
		local cframe =
			PlacementUtils.GetSnappedAttachmentCFrame(tile, snappedPoint, placeableInfo, state)
	...
	if state._is_stacked then
		structure_instance:SetAttribute(
			"StackedOn",
			state._stacked_structure:GetAttribute("PlotId")
		)
	end
end
